-- Apply new default nickname rule (adjective-noun-3digits) to all non-verified users
-- Adjust table/column/condition if needed (table: users, nickname column: display_name, unverified flag: is_verified)
...
create or replace function normalize_nickname() returns trigger language plpgsql as $$
begin
  if new.display_name is null
     or new.display_name ~* '^[0-9a-f-]{20,}$'
     or new.display_name ~* '^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$'
     or new.display_name ~* '^[^@\\s]+@[^@\\s]+\\.[^@\\s]+$'
     or length(new.display_name) < 2 then
    new.display_name := gen_default_nickname();
  end if;
  return new;
end;
$$;
...
-- Reset nicknames for all non-verified users (adjust condition if your flag differs)
update users
set display_name = gen_default_nickname()
