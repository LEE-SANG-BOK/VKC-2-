...
test.describe('functional flows (E2E_TEST_MODE)', () => {
  ...
  test('authenticated user can follow, like, and bookmark from feed', async ({ page, context }) => {
    ...
    const followButton = firstCard.getByRole('button', { name: '팔로우' });
  ...
  test('authenticated user can create, edit, and delete a post (UI)', async ({ page, context }, testInfo) => {
    ...
    await editor.click();
    await page.keyboard.type('E2E 본문 내용입니다.');

    await page.locator('form button[type="submit"]').click();
    await page.waitForURL(/\/ko\/posts\/.+/, { timeout: 30_000 });
    ...
    const editEditor = article.locator('.ProseMirror').first();
    await editEditor.click();
    await page.keyboard.press('ControlOrMeta+A');
    await page.keyboard.type('수정된 본문 내용입니다.');
