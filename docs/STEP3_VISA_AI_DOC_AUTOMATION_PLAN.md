...
- 비자 “전체 커버” 리스트(SoT)
  - 대상 비자 타입 목록(코드/명칭) + 최소한의 분류(유학/취업/거주/동반/기타)
  - 각 비자별 v1 룰셋을 만들 “공식 출처 URL”(HiKorea/법무부/고시/서식 게시판 등)
- 문서 템플릿 원본
  - `통합신청서`와 `유학생 알바(시간제취업허가/신고)`의 최신 **공식 PDF 원본 파일/링크**
  - 가능하면 “fillable PDF(AcroForm)”인지 여부(필드명이 있으면 템플릿 작업이 급격히 빨라짐)
- 공식 정보 동기화 대상 소스(최소 3개)
  - “무조건 봐야 하는” 페이지/게시판 URL 3~10개(변경 빈도, 우선순위 포함)
  - 업데이트 주기(일/주/월) 기대치
- 리드(핫리드) 운영 정의
  - 핫리드 임계치(예: 확률 ≥ 80%) + 어떤 액션을 득점으로 볼지(평가 실행/서류 생성/상담 버튼 클릭 등)
  - 관리자 화면에서 꼭 보고 싶은 컬럼/필터(상태, 지역, 목표비자 등)
