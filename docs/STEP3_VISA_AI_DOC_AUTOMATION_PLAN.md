...
> STEP3에서 사용하는 “공식 링크/템플릿/비자 목록/리드 기준” SoT는 `docs/STEP3_SOT_RESOURCES.md`로 고정한다.
...
- 포함(지금 단계에 “도입”)
  ...
  - PDF 자동 생성: **통합신청서(신고서) + 유학생 시간제취업(알바) 패키지**
    - 통합신청서(신고서).pdf
    - 시간제취업확인서.pdf
    - 유학생 시간제취업 요건 준수 확인서(제조업_국문).pdf (조건 해당 시)
...
## 3.3 기술 스캐닝 결론(오픈소스/구현전략 선택)

STEP3의 현실적인 1차 목표는 “규정/템플릿 기반으로 정확히 ‘작동’하는 확률/서류/리드/동기화”를 만드는 것이다. ML은 데이터가 쌓인 이후 고도화로 분리한다.

- 비자 확률 평가(룰엔진 우선, ML은 2차)
  - v1(권장): Node 런타임에 적합한 JSON 룰엔진(`json-rules-engine` 등) + DB 룰셋(`visa_rulesets`)로 구현
    - 장점: Next.js API Routes에서 바로 실행 가능, 룰 변경/버전 관리가 쉽고 “전 비자 타입 커버”를 선형 확장 가능
  - v2(선택): 데이터 축적 후 ML 서비스(FastAPI + XGBoost/LogReg 등)로 “확률 보정/보조”만 추가(단독 판정 금지)
    - 필요조건: 승인/거절/보완요청 등 outcome 데이터 축적(동의 포함) + 모델/설명가능성(피처/기여도) + 운영 모니터링
- PDF 자동 생성(템플릿 엔진 고정)
  - v1(권장): `pdf-lib` 기반(브라우저/서버 모두 가능)
    - 가능하면 AcroForm(fillable PDF) 채우기 + flatten
    - 아니면 배경 템플릿 + 좌표(renderSpec) 오버레이(폰트 임베딩 포함)
  - 템플릿 작성 가속(선택): `pdfme` 같은 시각 템플릿 에디터를 “내부 툴”로 사용해 renderSpec 생성(프로덕션 런타임 의존은 최소화)
  - STEP3 템플릿 SoT(링크/파일Id)는 `docs/STEP3_SOT_RESOURCES.md`를 따른다.
- OCR 입력 자동화(선택적 보조)
  - v1(권장): 여권 MRZ 인식(체크섬 검증 가능) → Wizard 자동 채움
  - v2(선택): ARC/기타 카드 OCR은 별도 OCR 서비스(FastAPI + Tesseract/EasyOCR 등)로 분리(주소는 오류 가능성이 높아 “보조”로만)
  - 보안(권장): OCR 원문/로그는 기본 마스킹(예: 여권번호/등록번호 일부) — 필요 시 Presidio 같은 PII redaction을 “로그/분석용”으로만 적용
- 관리자 리드/영업 자동화(경량 자체 구현 우선)
  - v1(권장): 내부 리드 점수/핫리드 플래그를 DB/관리자 UI로 구현(이벤트 기반 득점 + 임계치)
  - v2(선택): 외부 CRM/마케팅 자동화(Mautic 등) 연동은 필요 시 API로만 연결(도입 자체는 운영/배포 복잡도 큼)
- 공식 정보 동기화(크롤링→검토→승격)
  ...
  - v2.1(선택): data.go.kr OpenAPI로 “출입국/체류 통계”를 정형 데이터로 동기화(콘텐츠/리포트/대시보드에 활용)
...
### 3.5 이번 조사 내용 반영 체크(요약)

- 채택(플랜에 포함)
  - ML 기반 확률(서비스)은 v2로 “추가 가능”하도록 설계(SoT는 룰셋 v1)
  - PDF 생성은 템플릿 기반(정적 PDF 오버레이) + 내부 툴(pdfme)로 템플릿 제작 속도 확보
  - OCR은 Phase 2.5로 명시(여권 MRZ 우선) + 로그/분석용 PII 마스킹 고려
  - 리드 자동화는 내부 스코어링 v1 + 외부 CRM 연동은 옵션(v2)
  - 공식 정보 동기화는 크롤링+승인 파이프라인 v1 + data.go.kr 통계 API는 옵션(v2.1)
...
- [ ] `unified`/`parttime` PDF 생성 + Storage 저장 + 다운로드
  - `parttime`은 “패키지(2~3개 PDF)”를 기본으로 한다: (A) 통합신청서(신고서) + (B) 시간제취업확인서 + (C) 제조업 해당 시 요건 준수 확인서
- [ ] 서류함 UI + 이력/다운로드
...
## 13) 내가 추가로 필요한 정보(네가 조사해서 주면 바로 고정 가능)

아래 4개는 제공 완료(SoT: `docs/STEP3_SOT_RESOURCES.md`).

- [x] 비자 “전체 커버” 리스트(Registry)
- [x] 문서 템플릿 원본(HiKorea 민원서식) + 다운로드 방식/파일Id
- [x] 공식 정보 동기화 대상 소스(A/B/C)
- [x] 리드(핫리드) 운영 정의(핫리드 기준/컬럼/필터)

남은 “결정/확정”만 추가로 필요:

- PDF 패키지 다운로드 포맷: `zip(권장)` vs `PDF 개별 다운로드`(둘 다 가능, UX 우선순위 결정)
- “비자 전체 커버” 룰셋 v1 작성 순서(우선순위): 타겟 페르소나 기준으로 `D-2 → D-10 → E-7 → F-2`를 1차로 고정할지 여부
