...
export default function GuidelinesModal({ isOpen, onClose, title, items, closeLabel, confirmLabel }: GuidelinesModalProps) {
  const cleanedItems = useMemo(() => items.map((item) => item.trim()).filter(Boolean), [items]);

  return (
    <Modal isOpen={isOpen} onClose={onClose} title={title} closeLabel={closeLabel} maxWidth="max-w-lg">
