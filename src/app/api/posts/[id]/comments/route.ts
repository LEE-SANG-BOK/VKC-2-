...
import { setPrivateNoStore, successResponse, errorResponse, notFoundResponse, unauthorizedResponse, serverErrorResponse, paginatedResponse, rateLimitResponse } from '@/lib/api/response';
...
export async function GET(request: NextRequest, context: RouteContext) {
  try {
    ...
    if (isE2ETestMode()) {
      ...
      const flat = Array.from(store.comments.values()).filter(
        (comment) => comment.postId === postId && !comment.answerId
      );
      const tree = buildE2ECommentTree(store, flat);
      ...
      const data = tree.slice(offset, offset + limit);
  ...
  try {
    ...
    if (isE2ETestMode()) {
      ...
      if (parentId) {
        ...
        if (parentComment.postId !== postId || parentComment.answerId) {
      ...
      const created = createComment(
        ...
        namespace,
        user.id,
        { postId, answerId: null, parentId: parentId ? String(parentId) : null },
        normalizedContent
      );
