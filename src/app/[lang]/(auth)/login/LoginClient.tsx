...
export default function LoginClient({ lang, translations }: LoginClientProps) {
  const router = useRouter();
  const { data: session, status } = useSession();

  const t = (translations?.login || {}) as Record<string, string>;
  const title = t.title || '';
  const welcome = t.welcome || '';
  const googleLogin = t.googleLogin || '';
  ...
  return (
    <div className="min-h-screen min-h-[100dvh] flex items-center justify-center bg-gray-50 dark:bg-gray-900 px-4">
